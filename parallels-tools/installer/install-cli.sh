...
# To skip installation of prl_fs. For testing purposes ONLY
#SKIP_PRLFS_KMOD_INSTALLATION=true
...
install_kmods_src() {
	...
	if [ -n "$SKIP_PRLFS_KMOD_INSTALLATION" ]; then
		MAKE_OPTS+=" PRL_FS_SKIP=1"
	fi

	make -C "$INSTALL_DIR_KMODS" -f Makefile.kmods $MAKE_OPTS
	...
	for kmod_path in "${KMODS_PATHS[@]}"; do
		...
		if [ "$kernel_module_name" = 'prl_fs' ] && [ -n "$SKIP_PRLFS_KMOD_INSTALLATION" ]; then
			continue
		fi
...
remove_tools_modules() {
	...
	IFS=$'\n'
	awk '$3 == "prl_fs" || $3 == "fuse.prl_fsd" {print $2}' /proc/mounts |
...
install_tools_modules() {
	...
	# Install shared folders FUSE user-mode drivdaemoner
	local sffuse="$TOOLS_BIN_DIR/bin/prl_fsd"
	install_file "$sffuse" "$BIN_DIR/prl_fsd"
	...
	# Man-page for prl_fs
	if [ -z "${SKIP_PRLFS_KMOD_INSTALLATION}" ]; then
