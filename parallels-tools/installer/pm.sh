...
check_debian_package_installed() {
    local package=$1
		...
		apt-get)
			...
			do
				...
				if [ $need_check_src -eq 1 ]; then
					msg=$(
						DEBIAN_FRONTEND=noninteractive apt-get install -y -q --print-uris $pkg | \
						grep "^'cdrom:\[" | \
						sed "s/^'cdrom:\[\(.*\)\].*/\1/" | \
						while read label; do
				...
				test $rc -eq 0 && DEBIAN_FRONTEND=noninteractive apt-get -y -q --allow-unauthenticated install "$pkg" \
					>&3 2>&1
				rc=$?
...
check_deps()
{
	...
	if [ "$prod" = 'gtools' ]; then
		...
		if [ "$os_name" = 'debian' ] && [ "$karch" = 'aarch64' ]; then
			check_debian_package_installed binfmt-support || \
				deps_list="${deps_list:+${deps_list}\n}o binfmt-support"
		fi
		...
		type fusermount > /dev/null 2>&1 ||
			deps_list="${deps_list:+${deps_list}\n}m fuse2"
		# For ubuntu fuse/fuse3 conflicts with each other. 
		# workaround is to inexplicitly install libfuse2
		# see https://askubuntu.com/questions/1404988/fuse-and-fuse3-can-i-install-both	
		# But on the latest Ubuntu, libfuse2 is a proxy virtual package, so we need to check
		# the presence of the real package libfuse2t64.
		if [ "$os_name" = 'debian' ]; then
			if ! check_debian_package_installed libfuse2 && ! check_debian_package_installed libfuse2t64; then
				deps_list="${deps_list:+${deps_list}\n}m libfuse2"
			fi
		fi
	fi
...
case $action in
	...
	install)
		...
		failed_opt_dep=""
		for dep in $opt_deps_list; do

			# Split installation of each optional package to the separate call to PM
			# to ensure that all of packages will be installed (even if installotion 
			#of previous package failed) 
			pm_install $pm "$dep" "$logfile"
			rc=$?
